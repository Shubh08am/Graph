  ...
  public:
    vector<int> numOfIslands(int n, int m, vector<vector<int>> &operators) {
          ...
          for(auto& it : operators){
              ...
              //if all already visited 
              if(vis[row][col]){
                  //take components no change in components 
                  ans.push_back(components);
              }
              else if(!vis[row][col]){
                  vis[row][col]=1; //mark visited 
                  components++; //increase components by 1 
                  
                  int dx[] = {-1,0,+1,0};
                  int dy[] = {0,-1,0,+1};
                  
                  //traverse all 4 directions and see for common side 
                  for(int ind = 0 ; ind<4 ; ind++){
                      ...
                      if(adjRow>=0 && adjRow<n && adjCol>=0 && adjCol<m){
                          //island exist than it should have been marked visited 
                          if(vis[adjRow][adjCol]==1){
                          //if already connected than same ultimate parent else connect 
                          int nodeNo = row*m+col ; 
                          int adjNodeNo = adjRow*m+adjCol;
                          
                         //not connected(diff U.P)and island exist than do connection and reduced components
                         if(ds.findUltimateParent(nodeNo)!=ds.findUltimateParent(adjNodeNo)){
                             components--;
                             //make connection between adjNodeNo & nodeNo
