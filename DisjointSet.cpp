...
int main(){
   //don't use both union by rank and size on same graph at same time 
   DisjointSet ds(7) ; //7 nodes 
    ...
    //if u,v nodes belong to same components i.e same ultimate parent exist 
    //using findUltimateParent to check
    if(ds.findUltimateParent(1) == ds.findUltimateParent(7)){
        //3,7 belongs to same components or not 
        cout<<"same components\n";
    }
    else{
         cout<<"Not same components\n";
    }
